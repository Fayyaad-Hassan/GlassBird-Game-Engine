...
namespace GlassBird
{
	ShaderOpenGL::ShaderOpenGL(const std::string& vertexFile, const std::string& fragmentFile)
	{
		...
		if (!success)
		{
			...
			GLASSBIRD_ERROR("ERROR::SHADER::VERTEX::COMPILATION_FAILED\n" << infoLog);
		}
		...
		if (!success)
		{
			...
			GLASSBIRD_ERROR("ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLog);
		}
		...
		if (!success)
		{
			...
			GLASSBIRD_ERROR("ERROR::SHADER::PROGRAM::LINKING_FAILED\n" << infoLog);
		}
